    ...
    <section id="setlist" className="relative px-4 py-24 md:px-6">
      <div className="mx-auto max-w-3xl">
        <Reveal className="text-center">
          ...
          <p className="mt-4 leading-relaxed text-muted-foreground">{playlist.subtitle}</p>
        </Reveal>
          ...
          {playlist.tracks.map((track, i) => {
            ...
            return (
              <Reveal key={track.title} delay={i * 80}>
                <div
                  className={`flex items-center gap-4 border-b border-border/60 px-4 py-4 transition-colors last:border-b-0 md:px-6 ${
                    isPlaying ? "bg-primary/10" : "hover:bg-muted/40"
                  }`}
                >
                  <button
                    type="button"
                  ...
                  >
                    {isPlaying ? <FaPause className="h-3.5 w-3.5" /> : <FaPlay className="h-3.5 w-3.5 translate-x-px" />}
                  </button>
                    ...
                    <div className="flex items-end gap-0.5" aria-hidden="true">
                      {[0, 1, 2, 3].map((b) => (
                        <span
                          ...
                          style={{ height: "16px", animationDelay: `${b * 0.15}s` }}
                        />
                      ))}
                    </div>
                  ) : (
