...
import { GiGuitar, GiSpiderWeb } from "react-icons/gi";
...
export function HeroSection() {
  ...
  return (
    <section
      id="inicio"
      className="relative flex min-h-svh items-center overflow-hidden px-4 pt-24 pb-16 md:px-6"
    >
      <div className="mx-auto grid w-full max-w-6xl items-center gap-12 md:grid-cols-2">
        <div className="text-center md:text-left">
          ...
          <h1 className="mt-6 font-display text-5xl font-bold uppercase leading-[0.95] tracking-tight text-balance sm:text-6xl md:text-7xl">
            ...
            <span className="animate-glitch block text-primary text-glow">
              {hero.name}
            </span>
          </h1>
        ...
        <div className="relative mx-auto aspect-square w-full max-w-sm">
          ...
          <div className="absolute inset-4 rounded-full border border-secondary/40 animate-pulse-ring [animation-delay:0.8s]" />
          <div className="absolute inset-8 rounded-full border border-accent/40 animate-pulse-ring [animation-delay:1.6s]" />

          <GiSpiderWeb
            className="animate-spin-slow absolute inset-0 h-full w-full text-primary/20"
            aria-hidden="true"
          />

          <div className="animate-float-slow absolute inset-[10%]">
            <PhotoFrame
              src={hero.heroImage}
          ...
          <div
            className="absolute -bottom-2 left-1/2 flex -translate-x-1/2 items-end gap-1"
            aria-hidden="true"
          >
            {[0, 1, 2, 3, 4, 5, 6].map((i) => (
              <span
